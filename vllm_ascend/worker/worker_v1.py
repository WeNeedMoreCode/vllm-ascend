...
class NPUWorker(WorkerBase):
    ...
    def determine_available_memory(self) -> int:
        ...
        print(f"[ASCEND OLD] Memory Analysis:")
        print(f"[ASCEND OLD]   total_npu_memory: {total_npu_memory} bytes ({total_npu_memory/GiB:.2f} GiB)")
        print(f"[ASCEND OLD]   gpu_memory_utilization: {self.cache_config.gpu_memory_utilization}")
        print(f"[ASCEND OLD]   peak_memory: {peak_memory} bytes ({peak_memory/GiB:.2f} GiB)")
        print(f"[ASCEND OLD]   raw calculation: {total_npu_memory} * {self.cache_config.gpu_memory_utilization} - {peak_memory} = {raw_memory}")

        available_kv_cache_memory = int(raw_memory)
        available_kv_cache_memory = int(max(available_kv_cache_memory, 0))

        print(f"[ASCEND OLD] available_kv_cache_memory: {available_kv_cache_memory} bytes ({available_kv_cache_memory/GiB:.2f} GiB)")
